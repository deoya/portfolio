...
public class db_select {
	...
	@SuppressWarnings("unchecked")
	public JSONArray api_footer() {
		JSONArray ja = new JSONArray(); 
		JSONObject jo = new JSONObject(); 
		try {
			String sql = "select * from homepg_info";
			this.ps = this.con.prepareStatement(sql);
			this.rs = this.ps.executeQuery();


			while(this.rs.next()) {
				ja.add(this.rs.getString("hcompany"));
				ja.add(this.rs.getString("hcompany_num"));
				ja.add(this.rs.getString("hceo"));
				ja.add(this.rs.getString("hceo_hp"));
				ja.add(this.rs.getString("hreport_num"));
				ja.add(this.rs.getString("hpost_code"));
				ja.add(this.rs.getString("haddr"));
				ja.add(this.rs.getString("hinfo_officer"));
				ja.add(this.rs.getString("hinfo_email"));
				if(this.rs.getString("hprovide_num") != "") {
					ja.add(this.rs.getString("hprovide_num"));
				}
